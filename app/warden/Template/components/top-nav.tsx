...
import { Bell, MessageSquare, ChevronDown, User, LogOut, AlertCircle, Calendar } from 'lucide-react';
...
import { useSidebar, SidebarTrigger } from '@/app/warden/Template/components/ui/sidebar';
...
export function TopNav() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (role === 'admin') {
        setProfile({ name: 'System Admin', email: 'admin@bwf.org', role: 'Admin', profilePic: 'https://ui-avatars.com/api/?name=System+Admin&background=3b82f6&color=fff&rounded=true&bold=true' });
        return;
      }
      ...
      if (res.ok) {
        ...
        setProfile({...data, role: 'Warden'});
      } else {
        setProfile({ name: 'Warden', email: 'warden@school.com', role: 'Warden', profilePic: 'https://i.pravatar.cc/150?img=11' });
      }
    } catch (err) {
      console.error(err);
      setProfile({ name: 'Warden', email: 'warden@school.com', role: 'Warden', profilePic: 'https://i.pravatar.cc/150?img=11' });
          ...
          <DropdownMenu>
                ...
                {!profile ? (
                  ...
                  <>
                    <div className="w-10 h-10 rounded-full bg-slate-200 overflow-hidden shrink-0">
                      <img src={profile.profilePic || getAvatarUrl(profile.name || "Warden Admin")} alt="Warden" className="w-full h-full object-cover" />
                    </div>
                    <div className="hidden md:flex flex-col items-start mr-2">
                      <span className="text-[15px] font-bold text-slate-900 leading-tight tracking-tight">{profile.name}</span>
                      <span className="text-[13px] font-medium text-slate-500 leading-tight mt-0.5">{profile.role || 'Warden'}</span>
                    </div>
            ...
            <DropdownMenuContent align="end" className="w-64 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 duration-200 rounded-2xl border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] p-2 origin-top-right">
              <div className="px-2 py-3 mb-1">
                <p className="text-[15px] font-bold text-slate-900 leading-none">{profile?.name || 'Warden'}</p>
                <p className="text-[13px] font-medium text-slate-500 mt-1.5">{profile?.email || 'warden@school.com'}</p>
              </div>
              
              <DropdownMenuItem 
                onSelect={() => {
                  const role = localStorage.getItem("role");
                  router.push(role === 'admin' ? '/admin/profile' : '/warden/profile');
                }}
